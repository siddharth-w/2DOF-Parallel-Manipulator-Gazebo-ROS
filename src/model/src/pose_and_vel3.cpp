  ...
  class ModelJointControler : public ModelPlugin
  {
    public: void Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
    {
	  ...
	  const auto &jointController = this->model->GetJointController();
          ...
          jointController->AddJoint(model->GetJoint("front_right"));
          jointController->AddJoint(model->GetJoint("front_left"));

          jointController->AddJoint(model->GetJoint("joint1"));
          jointController->AddJoint(model->GetJoint("joint2"));

          this->right_wheel_name = model->GetJoint("front_right")->GetScopedName();
          this->left_wheel_name = model->GetJoint("front_left")->GetScopedName();
          
          this->joint_name1 = model->GetJoint("joint1")->GetScopedName();
          this->joint_name2 = model->GetJoint("joint2")->GetScopedName();
